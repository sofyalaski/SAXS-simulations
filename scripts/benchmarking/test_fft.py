...
def run_spheres(size, dtype):
    ...
    if size>=729:
        ...
        split_time, (split_mem, split_fft) = timer_wrapper(simulation.calculate_custom_FTI,three_d = True, device = 'cuda', smallest_memory=True,  dtype = dtype, memory_stats = True)
    simple_split_time, (simple_split_mem, simple_split_fft) = timer_wrapper( simulation.calculate_custom_FTI, three_d = True, device = 'cuda',  dtype = dtype, memory_stats = True)
    torch_time, (torch_mem, torch_fft)  = timer_wrapper(simulation.calculate_torch_FTI, device = 'cuda', dtype = dtype, memory_stats = True) 
    if torch_fft ==0:
        torch_time =0
    ...
    one_run.to_csv('../results/results_fft_all_1.csv', mode = 'a', header =False, index = False)
